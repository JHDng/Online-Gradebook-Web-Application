 ...
 */
@WebServlet("/GetDatesProfessor")
public class GetDatesProfessor extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Checks for string safety and parses it
		if(SafeChecker.isStringSafe(courseID_str)) {
			...
			// Default value so no dates are found in case of wrong courseID
			courseID = -1;
		}
		
		List<ExamDate> dates;
		try {
			// Finds dates with provided courseID
			dates = eDAO.findDatesByCourseID(courseID, ((User) request.getSession().getAttribute("user")).getUserUUID());
		
			if(dates == null || dates.isEmpty()) { // not found case
